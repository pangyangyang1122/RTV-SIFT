...
for i=1:1:iterations
    ...
    match1_xy=cor1(:,1:2)';     %size = 2 X M
    match1_xy=[match1_xy;ones(1,M)];          %size = 3 X M
    t_match1_xy=solution*match1_xy;           %变换后最后一行仍是全为1
    match2_xy=cor2(:,1:2)';
    match2_xy=[match2_xy;ones(1,M)];
    diff_match2_xy=t_match1_xy-match2_xy;
    diff_match2_xy=sqrt(sum(diff_match2_xy.^2));  %size = 1 X M
...
%Delete duplicate point pair   去除一对多的点
uni1=cor1_new(:,[1 2]);
