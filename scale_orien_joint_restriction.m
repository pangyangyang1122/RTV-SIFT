...
unit_x=5.5;
unit_y=5.5;
[S,RMO,~,~,~,~,~,~,~,~]=S_RMO_StoR(cor1,cor2,unit_scale,unit_angle,unit_x,unit_y);
...
%             error_S=error_S';
            ...
            JD=(1+T_error).*(1+error_RMO).*distance(k,:);
            [vals,index] = sort(JD); 
            ...
            if(i==1 && j==1)
                match(k,:)=match(k,:); % 第一次配就填进去
            elseif(prev_match(k,1)==0) % 该关键点没有对象，也填进去
                match(k,:)=match(k,:);
            elseif(prev_match(k,1)~=0) % 该关键点已有对象，就比一比，选好的对象
...
[end_solution,rmse,loc11,loc22]=FSC_2(loc11,loc22,change_form,1);%%Sub-pixel accuracy
